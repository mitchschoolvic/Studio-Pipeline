...
# Studio Pipeline - Server Startup Script
...
echo "🚀 Starting Studio Pipeline Servers..."
echo ""
...
cd "$BACKEND_DIR"
nohup "$VENV_PATH/bin/python" -m uvicorn main:app --host 0.0.0.0 --port 8000 --log-level info > /tmp/backend.log 2>&1 &
BACKEND_PID=$!
...
if [ -z "$REUSE_FRONTEND" ]; then
    VITE_APP_DEV_MODE="true" nohup ./node_modules/.bin/vite --host 0.0.0.0 --port 5173 > /tmp/vite_server.log 2>&1 &
    FRONTEND_PID=$!
